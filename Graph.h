...
const double pi = 3.14159;								//Regular_polygon 위한 pi 추가
...
struct Box : Shape {									//Box추가
	Box(Point p, int wid, int hei, double rad)
		:w{ wid }, h{ hei }, r{ rad }{
		add(Point{ p.x,p.y });
	}
...
struct Regular_polygon : Shape {						//Regualr_polygon추가
	Regular_polygon(Point p,int sides,double rad)
		:s{ sides }, r{ rad }{
		add(Point{ p.x,p.y });
		theta = (2*pi) / sides;
	}
...
struct Star : Shape {
	Star(Point p,int dots,double sr,double br)
		:n{ dots }, r1{ sr }, r2{ br }
	{
		add(Point{ p.x, p.y });
		theta = pi / dots;						//원래 2pi/dots지만 theta/2 단위로 바깥쪽 원, 안쪽 원 을 번갈아 가야함
	}
	...
	int sides() const { return n; };
	Point center() const { return Point{ point(0).x,point(0).y }; };
private:
	double r1;
	double r2;
	double theta;
...
struct Rose : Shape {
	Rose(Point p, int nn, int dd, double rad)
		:n{ nn }, d{ dd }, r{ rad }
	{
		add(Point{ p.x,p.y });
		k = (double)nn / dd;
		if ((nn * dd) % 2) theta = pi * dd;	//nd is odd
		else theta = pi * dd * 2;
	}
...
struct Tschirnhausen_cubic : Shape {
	Tschirnhausen_cubic(Point p,int aa)
		:a{ aa }
	{
		add(Point{ p.x,p.y });
	}
