...
void Box::draw_lines() const
{
	if (fill_color().visibility()) {
		...
		fl_rectf(point(0).x, point(0).y + r, w, h - r - r);
		...
		fl_pie(point(0).x + w - 2 * r, point(0).y + h - 2 * r, r+r-1, r+r-1, 270, 360);
...
void Regular_polygon::draw_lines() const
{
	if (color().visibility()) {
		fl_color(color().as_int());
		for (int i = 0; i < s; i++)			//꼭짓점 개수만큼 반복
		{
			fl_line(point(0).x+r*cos(theta*i),point(0).y+r*sin(theta*i), 
				point(0).x + r * cos(theta * (i+1)), point(0).y + r * sin(theta * (i+1)));	//극좌표 계산
		}
	}
}
...
{	
	if (color().visibility()) {
		...
		for (int i = 0; i < n; i++)
		{
			fl_line(point(0).x + r2 * cos(2*theta * i), point(0).y + r2 * sin(2*theta * i),
				point(0).x + r1 * cos(theta * (2*i+1)), point(0).y + r1 * sin(theta * (2*i + 1)));
			fl_line(point(0).x + r1 * cos(theta * (2*i + 1)), point(0).y + r1 * sin(theta * (2*i + 1)),
				point(0).x + r2 * cos(2*theta*(i + 1)), point(0).y + r2 * sin(2*theta*(i + 1)));
		}
		/*
		for (int i = 2; i < number_of_points(); i++)	//point(0)은 중심좌표이므로 반복문을 i=2부터 실행
		{
			fl_line(point(i - 1).x, point(i - 1).y, point(i).x, point(i).y);
		}
...
void Rose::draw_lines() const
{
	...
	{
		fl_color(color().as_int());
		for (double i = 0; i < theta; i += (pi/90))
		{
			fl_line(point(0).x + r * cos(k * i) * cos(i), point(0).y + r * cos(k * i) * sin(i),
				point(0).x + r * cos(k * (i+(pi/90))) * cos(i+(pi/90)), point(0).y + r*cos(k * (i+(pi/90))) * sin(i+(pi/90)));
		}
	}
}

void Tschirnhausen_cubic::draw_lines() const
{
	if (color().visibility())
	{
		fl_color(color().as_int());
		for (double i = -pi; i < pi; i += (pi / 90))
		{
			fl_line(point(0).x + a*(1-3*tan(i/3)*tan(i/3)),point(0).y+a*(3*tan(i/3)-tan(i/3)*tan(i/3)*tan(i/3)),
				point(0).x + a * (1 - 3 * tan((i+(pi/90)) / 3) * tan((i + (pi / 90)) / 3)), point(0).y + a * (3 * tan((i + (pi / 90)) / 3) - tan((i + (pi / 90)) / 3) * tan((i + (pi / 90)) / 3) * tan((i + (pi / 90)) / 3)));
		}
	}
}
